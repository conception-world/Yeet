...
async fn main() -> Result<()> {
    ...
    // marker belongs to someone else — `rojo sourcemap --watch` is the standard
    // luau-lsp setup — and Yeet used to replace it, silently, on every start.
    let generate_sourcemap = if args.no_sourcemap {
        info!("--no-sourcemap: sourcemap.json will not be generated or maintained");
        false
    } else if sourcemap::is_foreign(&state_inner.root) {
        warn!(
            "sourcemap.json was not generated by Yeet (no `{}: {}` marker) — leaving it \
             untouched. Delete it to let Yeet manage the file, or pass --no-sourcemap \
             to silence this.",
    ...
    let sourcemap_rx = if generate_sourcemap {
        ...
        // `sourcemap_tx`; the writer debounces and regenerates the map. Seed
        // its baseline signature from the tree we just wrote so the first
        // structural change (not a stray content edit) triggers the next
        // rewrite. Leaving `sourcemap_tx` as `None` when generation is off
        // makes `mark_sourcemap_dirty` a no-op, so nothing downstream needs a
        // second switch.
