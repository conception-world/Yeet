...
struct CliArgs {
    ...
    dry_run: bool,
    /// `--no-sourcemap`: never generate or maintain `sourcemap.json`. For a
    /// project that already drives luau-lsp from `rojo sourcemap --watch` or a
    /// hand-maintained map. The extension passes this when
    /// `yeet.generateSourcemap` is off; the ownership marker
    /// (`sourcemap::is_foreign`) already protects a map Yeet did not write, so
    /// this flag is for users who want the feature off outright.
    no_sourcemap: bool,
}
...
async fn main() -> Result<()> {
    ...
    // `--no-sourcemap` is the ONLY hard switch: the user saying they drive
    // luau-lsp some other way. It leaves `sourcemap_tx` as `None`, which makes
    // `mark_sourcemap_dirty` a no-op, so nothing downstream needs a second
    // check.
    //
    // Foreign-map ownership is deliberately NOT a startup switch. It used to be
    // one, and that was a latch: a `sourcemap.json` without Yeet's marker at
    // boot disabled the channel for the whole session, so deleting the foreign
    // map mid-session did nothing and only a daemon restart recovered. The
    // ownership check lives in `sourcemap::write_value`, which re-runs it on
    // every single write — so arming the channel unconditionally is safe (a
    // foreign map is still never clobbered) and makes the state recoverable the
    // moment the user deletes the file.
    // Did the bootstrap write actually land? Seeds the background writer's
    // baseline: `Some(sig)` only when the map on disk really is ours, so a
    // bootstrap that was refused (foreign map) or errored leaves the baseline
    // unset and the first structural change retries instead of comparing equal
    // and skipping.
    let mut bootstrap_wrote = false;
    let sourcemap_rx = if args.no_sourcemap {
        info!("--no-sourcemap: sourcemap.json will not be generated or maintained");
        None
    } else {
        match sourcemap::write_sourcemap(
            &state_inner.root,
            &state_inner.project,
            &state_inner.tree_fs,
            &state_inner.package_remaps,
        ) {
            Ok(sourcemap::WriteOutcome::Wrote) => bootstrap_wrote = true,
            Ok(sourcemap::WriteOutcome::SkippedForeign) => {
                warn!(
                    "sourcemap.json was not generated by Yeet (no `{}: {}` marker) — leaving \
                     it untouched. Delete it to let Yeet manage the file (no restart needed), \
                     or pass --no-sourcemap to silence this.",
                    sourcemap::GENERATED_BY_KEY,
                    sourcemap::GENERATED_BY_VALUE,
                );
            }
            Err(e) => warn!(error = ?e, "failed to write initial sourcemap.json"),
        }
        // Wire the background writer: structural tree changes nudge
        // `sourcemap_tx`; the writer debounces and regenerates the map.
        let (sourcemap_tx, sourcemap_rx) = mpsc::unbounded_channel::<()>();
        state_inner.sourcemap_tx = Some(sourcemap_tx);
        Some(sourcemap_rx)
    };
    let sourcemap_sig = bootstrap_wrote
        .then(|| sourcemap::structure_signature(&state_inner.project, &state_inner.tree_fs));
    if args.dry_run {
        warn!(
    ...
    if let Some(sourcemap_rx) = sourcemap_rx {
...
///
/// `last_sig` is `Option` and only advances on a write that actually reached
/// disk. It starts as `None` when the bootstrap write did not land (a foreign
/// map, or an I/O error), which forces the next wake-up to attempt a write
/// regardless of the signature. Advancing it unconditionally — as this used to
/// — meant a refused write still marked the structure as "done", so deleting a
/// foreign `sourcemap.json` mid-session never brought the map back: every later
/// cycle compared equal and returned before touching the disk.
async fn sourcemap_writer(
    state: SharedState,
    mut rx: mpsc::UnboundedReceiver<()>,
    mut last_sig: Option<u64>,
) {
    while rx.recv().await.is_some() {
        ...
        drop(guard);
        match sourcemap::write_value(&root, &value) {
            Ok(sourcemap::WriteOutcome::Wrote) => last_sig = Some(sig),
            // Leave `last_sig` untouched so the next structural change retries.
            // `write_value` already logged why it skipped.
            Ok(sourcemap::WriteOutcome::SkippedForeign) => {}
            Err(e) => warn!(error = ?e, "failed to write sourcemap.json"),
        }
    }
}
